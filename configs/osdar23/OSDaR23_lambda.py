...
# ---- misc custom setting ----------------
sweep=False # Set by default to false. If you wnat to run sweep, use the --options sweep==True optionz
...
# In the type arguement below, the name of the transforms we want to apply on the added samples must be put into a list
# e.g. type=["PolarMixPaste","SparsifyTrackIgnore"], augment_ratio=0.5 to create 50% of added samples with both transforms applied or
#      type=["PolarMixPaste"], augment_ratio=1 to create 100% of added samples with one single transform
# Note that the transforms should be properly defined in data/train/transform below.
augmented_sampler = dict(active = True,
                         augmentations_list = [
                                               #dict(type=["PolarMixPaste"], augment_ratio=0.9),                             # If there are 100 samples, with one augm. with augment_ratio=0.4 -> 140 samples
                                               dict(type=["Sparsify"], augment_ratio=0, start_epoch=25),
...
upload_pcd_every_x_epoch = 5 # The pcd will be uploaded every x epoch
...
data_root = "/baseline/preprocessed_pcd"
#data_root = "/workspaces/baseline/data/OSDaR_dataset/v_2" 
...
data = dict(
    ...
    train=dict(
        ...
        data_root=data_root,
        transform=[
            #dict(type="PolarMixPaste", p=.0, csv_stat_path="/baseline/railseg/csv_stats/pedestrian_density_per_distance.csv"),
            ...
            #dict(type="SparsifyTrackIgnore", end_range=80, track_label=learning_map["track"],p=0.),
...
hooks = [
    ...
    dict(type="CheckpointSaver", save_freq=None, save_best_at=None), # set save_best_at value to save the best model up to this epoch
