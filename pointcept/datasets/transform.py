...
class Collect(object):
    ...
    def __call__(self, data_dict):
        ...
        for name, keys in self.kwargs.items(): # Here is created the "feat" in input_dict
...
class RandomFlip(object):
    def __init__(self, p=0.5, around_xaxis_only=False):
        self.p = p
        self.around_xaxis_only = around_xaxis_only

    def __call__(self, data_dict):
        if self.around_xaxis_only==False:
...
class GridSample(object):
    ...
    def __call__(self, data_dict):
        ...
        _, inverse, count = np.unique(key_sort, return_inverse=True, return_counts=True) # Count provide the number of point in each grid cell
        if self.mode == "train":  # train mode
            ...
            for i in range(count.max()):
                idx_select = np.cumsum(np.insert(count, 0, 0)[0:-1]) + i % count # Select the next combination of the points. If only one point exist in cell, alway the same.
...
class PolarMixPaste(object): # Code inspired from : https://github.com/xiaoaoran/polarmix
    ...
    def __init__(self, p=1, no_ground_pasting=True, apply_rotation=True, flip_prob=0.5, 
        ...
        if self.apply_rotation:
            self.random_rotate = RandomRotate(angle=[-1,1], axis="z", always_apply=True)
        self.flip_prob = flip_prob                  # Probability to apply a flipping of the instance
        self.horizontal_shift = horizontal_shift    # Whether to apply a random horizontal shift or not
    ...
    def create_person_density_stat(self, csv_stat_path):
        df = pd.read_csv(csv_stat_path)
        training_df = df[df["split"]=="train"]

        bucket_ranges = np.arange(0, training_df["dist"].max()+self.STAT_BUCKET_SIZE, self.STAT_BUCKET_SIZE) 
        # Assign each "dist" to its corresponding bucket, with the label defined by the left limit of each bucket -> groupby bucket -> take mean value per bucket 
    ...
    def __call__(self, data_dict):
        ...
        if (np.random.random() < self.p) or (self.__class__.__name__ in data_dict["add_samples_transform"]): #Second condition occur when the dataset size was artificialy augmented
            data_dict = self.instance_copy(data_dict)
    ...
    def instance_copy(self, data_dict):
        ...
        for i in person_instances:
            ...
            if len(pts_below_inst_idx)>0: # If no pts, person is in an unrealistic location -> disregard
                mean_z_1 = np.mean(data_dict["coord"][pts_below_inst_idx, 2]) # Find mean z location
            elif self.no_ground_pasting: # If no points are found under person in scene 1, paste at the average position of the track
                mean_z_1 = average_track_z_coord
            else:
                continue
        ...
        if found_element_to_copy: # Create boolean to count number augmentation where a pedestrian was indeed pasted.
            ...
            self.counter_successful_pasting += 1
       
        else:
            data_dict["pasting_successful"] = False
        
        return(data_dict)
    
    def get_number_successful_pasting(self):

        return self.counter_successful_pasting, self.counter_function_call
...
class Sparsify(object): # Method inspire by "Part-Aware Data Augmentation for 3D Object Detection in Point Cloud"
    ...
    def sparsify(self, data_dict):
        ...
        for key in ["coord", "strength", "segment", "instance", "grid_coord"]:
            if key in data_dict.keys():
