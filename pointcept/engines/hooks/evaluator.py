...
class SemSegEvaluator(HookBase):
    ...
    def eval(self):
        ...
        for i, input_dict in enumerate(self.trainer.val_loader):
            ...
            scene = str(round(input_dict["scene"][0].item(),1))
            frame = str(int(input_dict["frame"][0].item()))

            if self.trainer.cfg.pcd_to_wandb: # Register some example point cloud to wandb for visualisation
                if ( (current_epoch%self.trainer.cfg.upload_pcd_every_x_epoch) == 0 ) & \
                    ( (scene == "2.1") & (frame == "250") \
                     | (scene == "3.4") & (frame == "91") \
                     | (scene == "4.2") & (frame == "26") \
                     | (scene == "6.1") & (frame == "17") \
                     | (scene == "7.2") & (frame == "673") \
                     | (scene == "9.3") & (frame == "219") \
                     | (scene == "11.1") & (frame == "276") \
                     | (scene == "14.3") & (frame == "17") \
                     | (scene == "18.1") & (frame == "76") \
                     | (scene == "21.3") & (frame == "570")) : # Only for specific pcds
                    cpu_pred = pred.cpu().numpy()
                    cpu_segment = input_dict['segment'].cpu().numpy()
                    color_map_dict = self.trainer.cfg['data']['color_map']

                    pred_colors = np.array([color_map_dict[val] for val in cpu_pred]) 
                    gt_colors = np.array([color_map_dict[val] for val in cpu_segment])
                    np_pcd = input_dict['coord'].cpu().numpy()
                    np_pcd[:,1] = 0-np_pcd[:,1] # For some reason the y axis is inverted in wandb
                    self.trainer.writer.log({"prediction_pcd/scene_"+ scene + "_frame_" + frame + "/epoch_"+str(current_epoch): wandb.Object3D(np.c_[np_pcd, pred_colors])})

                    # Only uploading for the ground truth the first time:
                    if current_epoch == self.trainer.cfg.upload_pcd_every_x_epoch:
                        self.trainer.writer.log({"gt_pcd/scene_"+ scene + "_frame_" + frame + "/epoch_"+str(current_epoch): wandb.Object3D(np.c_[np_pcd, gt_colors]), "epoch":current_epoch})

            self.trainer.storage.put_scalar("val_loss", loss.item())
            
            update_distancewise_metric(input_dict["coord"], pred, segment, self.trainer.cfg, self.trainer.storage)
        ...
        try:  #TODO Create hooks for only uploading train metric to avoid this
            ...
            train_target = self.trainer.storage.history("train_target").total
            iou_class_train = train_intersection / (train_union + 1e-10)
            recall_class_train = train_intersection / (train_target + 1e-10)
            m_iou_train = np.mean(iou_class_train[self.trainer.cfg.data.evaluated_class]) # Evaluate solely on the class that we specify
            if self.trainer.writer is not None:
                self.trainer.writer.log({"epoch": current_epoch, "train/mIoU": m_iou_train})
        except:
            pass
