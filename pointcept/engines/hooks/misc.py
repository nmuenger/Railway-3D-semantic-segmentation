...
class InformationWriter(HookBase):
    ...
    def before_train(self):
        # When rebuilding the training from a certain step must compute the current step
        self.trainer.comm_info["iter_info"] = ""
        if type(self.trainer.train_loader.sampler).__name__!="AugmentedSampler":
            self.curr_iter = self.trainer.start_epoch * len(self.trainer.train_loader)
        else:
            self.curr_iter = self.trainer.train_loader.sampler.get_total_steps(self.trainer.start_epoch)
    ...
    def after_step(self):
        ...
        if self.trainer.writer is not None:
            ...
            #     )
            # NEW CODE
            self.trainer.writer.log({"iter":self.curr_iter, "lr": lr})
            for key in self.model_output_keys:
                self.trainer.writer.log({"iter":self.curr_iter, "train_batch/" + key: self.trainer.storage.history(key).val})
        ...
        if self.trainer.writer is not None:
            for key in self.model_output_keys:
                ...
                self.trainer.writer.log(
                    {"epoch":self.trainer.epoch + 1, "train/" + key: self.trainer.storage.history(key).avg})


@HOOKS.register_module()
class CheckpointSaver(HookBase):
    def __init__(self, save_freq=None, save_best_at=None):
        self.save_freq = save_freq  # None or int, None indicate only save model last
        self.save_best_at = save_best_at # None or int, at a given epoch, save the best model so far
    ...
    def after_epoch(self):
        if is_main_process():
            ...
            if self.save_best_at and (self.trainer.epoch + 1) % self.save_best_at == 0:
                shutil.copyfile(
                    os.path.join(self.trainer.cfg.save_path, "model", "model_best.pth"),
                    os.path.join(self.trainer.cfg.save_path,"model",f"best_up_to_epoch{self.trainer.epoch + 1}.pth"),
                )
...
@HOOKS.register_module()
class CUDAMemoryConsumption(HookBase):
    def __init__(self):
        self.largest_memory_allocated = 0
        self.largest_memory_reserved = 0 
        self.curr_iter = 0
    ...
    def after_step(self):
        ...
        mem_reserved_gb = mem_reserved/(1024**3)


        if self.trainer.writer is not None:
            self.trainer.writer.log({"iter": self.curr_iter, "memory_allocated": mem_allocated_gb, "memory_reserved": mem_reserved_gb})
            
        if mem_allocated_gb > self.largest_memory_allocated:
            self.largest_memory_allocated = mem_allocated_gb
...
class SuccessfulPastingCounter(HookBase):
    def __init__(self):
        self.nb_successful_pasting = 0
        self.nb_total_attempt = 0 

    def after_step(self):
        successful_pasting_flags = self.trainer.comm_info["input_dict"]["pasting_successful"].cpu().numpy()
        self.nb_successful_pasting += sum(successful_pasting_flags == True) 
        self.nb_total_attempt += sum(successful_pasting_flags != -1)
        
    def after_train(self):
        self.trainer.logger.info("Out of " + str(self.nb_total_attempt) + " total tries to apply paste transform, "\
                                 + str(self.nb_successful_pasting) + " resulted in a successful result.")