...
@TESTERS.register_module()
class SemSegTesterWandB(TesterBase):
    def test(self, experiment_name=None):
        ...
        wandb.init(project="Tests", name=experiment_name, mode=mode)
        writer = wandb
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            pred = pred.max(1)[1].data.cpu().numpy() # The pred is based on all the fragment inference. The label most given to a point ends up being the prediction
            ...
            update_distancewise_metric(torch.from_numpy(data_dict["entire_pcd_coord"]), pred, segment, self.cfg, storage)

        m_iou = log_metric(self.cfg,storage,logger,writer,0)
        
        print(f"Test result, mIoU: {m_iou}")
