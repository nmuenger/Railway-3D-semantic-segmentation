...
# Receives a point cloud and returns it in an appropriate format for wandb to display
def to_wandb_format(pcd, labels_list): 
    """TODO: Function in build
        pcd: a np array with the first three columns being x,y,z
        labels_list: list containing for each point of the point cloud the corresponding label (in str or number)"""
    
    rgb_values = labels_to_numpy_rgb(labels_list)


    wandb_pcd = np.c_[pcd[:,[0,1,2]], rgb_values]

    # Apparently the viewer of wandb seems to invert the y axis.
    wandb_pcd[:,1] = 0-wandb_pcd[:,1]
