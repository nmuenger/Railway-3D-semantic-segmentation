...
**configs/osdar23**: contains the config for running PTv3 with the OSDaR23 dataset <br>
**pointcept/datasets/osdar23.py**: script handling the loading of the data for OSDaR23. <br>
**railseg**: folder containing useful processing tool for our specific project (for example saving point cloud to a LAS file)<br>
**stat_analysis**: scripts to perform stats about the datasets (mainly OSDaR23)
**test_dev**: scripts to test different strategies, or analyse results afterward.

The data augmentation can operate in two ways, either by transforming training samples on the fly, or by actively creating new samples on top of the existing one, with the augmentation applied on it. 

For this to work, the transformation methodology must be coded in the script pointcept/datasets/transform.py.
Example of some configs, and in what they would result with a dataset of size 100.
...
```
Would result in 100 samples without the PolarMixPaste augmentation applied, plus 80 samples where PolarMixPaste would be applied on each.
...
```
Would result in 100 samples where the PolarMixPaste would be applied at 80% probability and Sparsify at 50% probability, plus 40 samples with the PolarMixPaste augmentation applied on each, plus 20 samples with the Sparsify augmentation applied on each.

When an augmentation is present in the augmentations_list, but its augment_ratio is 0, the specific augmentation is applied on all samples. This can be useful if we want to start applying a specific augmentation after a certain number of epoch, without augmenting the number of sample. Note that this behavious is deacivated when running a sweep TODO explain all reasoning more clearly
```python 
augmented_sampler = dict(active = True,
                         augmentations_list = [dict(type=["PolarMixPaste"], 
                                                    augment_ratio=0, start_at=25)])
...
```
Would result in 100 samples without the PolarMixPaste augmentation applied until reaching epoch 25, where the PolarMixPaste augmentation is applied at EVERY samples.

#### Below, rest of the pointcept README
----
<p align="center">
    <!-- pypi-strip -->
    <picture>
